...
st.markdown(
    ...
    <style>
    .stApp {
        background-color: #f4f6f7; /* soft light gray background */
    }
    .block-container {
        background-color: #fdfefe; /* off-white content area */
        border-radius: 8px;
        padding: 1rem;
    }
    </style>
...
# Refresh Button aligned right, separate from title
st.markdown(
    """
...
# STEP 7: Ensure 'date' column exists
...
# STEP 8: Sidebar Filters (previous month/year defaults)
...
# Map months to labels (Jan, Feb, etc.)
...
# Convert back to numeric month for filtering
...
if data_type == "Raw":
    ...
    # Hybrid Summary View (only shows when Summary is selected)
    view_choice = st.sidebar.radio("📊 Choose Summary View:", ["Excel Pivot", "Dynamic Summary"], index=0)

    if view_choice == "Excel Pivot":
        summary_pivot_df = pd.read_excel(uploaded_file or default_path, sheet_name="Summary_Pivot")
        st.markdown(
